...
library(tidyverse)
library(rvest) #librería de scrpaing
...
"28.028 Madrid" %>% str_replace('\\.', '') # es con expresiones regulares.
...
partidos <- html_madrid %>% html_nodes(".nombrePartido") %>% html_text() #etiquetas con clas = mombrePartido. 
concejales <- html_madrid %>% html_nodes(".tipoNumeroElectos") %>% html_text() %>% as.numeric #aquí lo convierte a número
...
#esta otra forma , si está bien hecha la tabla, como es el caso, lo podré hacer más rápido. 
